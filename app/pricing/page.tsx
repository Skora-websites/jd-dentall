...
export default function PricingPage() {
  ...
  const getPlanIcon = (iconName: string) => {
    switch (iconName) {
      case "Crown": return <Crown className="w-5 h-5 text-[#0D7A75]" />;
      case "Users": return <Users className="w-5 h-5 text-[#0D7A75]" />;
      case "HeartPulse": default: return <HeartPulse className="w-5 h-5 text-[#0D7A75]" />;
    }
  };
            ...
            <AnimateOnScroll animation="fadeUp">
              <div className="text-center mb-12">
                <div className="inline-flex p-1 rounded-full bg-slate-100 border border-slate-200 text-xs font-semibold">
                  <button onClick={() => setBillingCycle("monthly")} className={`px-5 py-2 rounded-full transition-all ${billingCycle === "monthly" ? "bg-[#0D7A75] text-white shadow-xs" : "text-slate-600 hover:text-[#101828]"}`}>
                    Monthly Plan (₹)
                  </button>
                  <button onClick={() => setBillingCycle("annual")} className={`px-5 py-2 rounded-full transition-all flex items-center gap-1.5 ${billingCycle === "annual" ? "bg-[#0D7A75] text-white shadow-xs" : "text-slate-600 hover:text-[#101828]"}`}>
                  ...
                  <div key={plan.id} className="pricing-card h-full">
                    <Tilt3DCard maxTilt={5} scale={isPopular ? 1.03 : 1.01} className="h-full">
                      <div className={`h-full rounded-3xl p-8 transition-all duration-300 flex flex-col justify-between relative bg-white ${
                        isPopular ? "border-2 border-[#0D7A75] shadow-2xl shadow-[#0D7A75]/15 ring-4 ring-[#0D7A75]/10" : "border border-slate-200/90 shadow-sm hover:shadow-xl hover:border-[#A3E3DF]"
                      }`}>
                        {isPopular && (
                          <div className="absolute -top-3.5 left-1/2 -translate-x-1/2 px-4 py-1 rounded-full bg-[#0D7A75] text-white text-xs font-bold uppercase tracking-wider shadow-md">Most Popular</div>
                        )}

                        <div className="space-y-6">
                          <div className="flex items-center gap-3">
                            <div className="w-12 h-12 rounded-2xl bg-[#E6F5F4] border border-[#A3E3DF]/60 flex items-center justify-center flex-shrink-0">{getPlanIcon(plan.iconName)}</div>
                            <div>
                          ...
                          <div className="pt-2 border-t border-slate-100">
                            ...
                            <p className="text-[11px] text-[#0D7A75] font-semibold mt-1">{priceInfo.savings} • Both Greater Noida clinics</p>
                          </div>
                        ...
                        <div className="pt-8">
                          <button onClick={() => setIsBookingOpen(true)} className={`w-full py-3 px-6 rounded-full font-bold text-sm transition-all flex items-center justify-center gap-2 shadow-xs ${
                            isPopular ? "bg-[#0D7A75] hover:bg-[#095C58] text-white shadow-md hover:shadow-lg" : "bg-white hover:bg-[#0D7A75] text-[#0D7A75] hover:text-white border-2 border-[#0D7A75]"
                          }`}>
              ...
              {faqs.map((faq, idx) => (
                <div key={idx} className="rounded-2xl bg-white border border-slate-200/90 p-6 hover:border-[#A3E3DF] hover:shadow-md transition-all">
                  <div className="flex items-start gap-3">
                    <HelpCircle className="w-5 h-5 text-[#0D7A75] flex-shrink-0 mt-0.5" />
                    <div>
